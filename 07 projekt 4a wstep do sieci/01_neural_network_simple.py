...
class NeuralNetworkRegression:
    ...
    def backward(self, X, y, output, learning_rate, reg_lambda) -> None:
        # Propagacja wsteczna błędu przez sieć
        output_error = y - output
        hidden_output = self.relu(
        ...
        self.weights_hidden_output += (
            gradient_hidden_output - reg_lambda * self.weights_hidden_output
        ) * learning_rate
        self.weights_input_hidden += (
            gradient_input_hidden - reg_lambda * self.weights_input_hidden
        ) * learning_rate
        self.bias_output += np.sum(output_error, axis=0) * learning_rate
        self.bias_hidden += np.sum(hidden_error, axis=0) * learning_rate

    def train(self, X, y, epochs, learning_rate, reg_lambda) -> None:
...
nn.train(
    X_train, y_train, epochs=epochs, learning_rate=learning_rate, reg_lambda=reg_lambda
)
...
plt.ylabel("Y")
plt.title(
    f"Neurons={hidden_size}, Epochs={len(nn.history_r2)}, learn_r={learning_rate}, lambda={reg_lambda}"
)
plt.legend()

# Zapisywanie wykresu
nazwa_pliku = now.strftime(
    f"DataSet_{XX}_%Y%m%d_%H%M%S_neu_{hidden_size}_epch_{len(nn.history_r2)}.png"
)
